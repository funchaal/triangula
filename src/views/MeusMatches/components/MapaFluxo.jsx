    ...
    <div className="flex-1 min-h-0 relative bg-[#0B1437]">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 px-4 md:px-5 pt-4 md:pt-5 pb-20 pointer-events-none">
        <div className="flex items-center gap-3 md:gap-4">
          {/* Container do Ícone: Ajustado para w-10 no mobile */}
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.3)]">
            ...
            <MapPin size={22} className="text-white hidden md:block" />
          </div>

          <div>
            {/* Título: de text-base para text-sm no mobile */}
            <div className="text-sm md:text-lg font-bold text-white drop-shadow-lg tracking-wide leading-tight">
              {selected ? cycleType(selected.chain) : 'Visão Geográfica'}
            </div>
            {/* Subtítulo: de 11px para 9px no mobile */}
            <div className="text-[9px] md:text-xs text-[#A3AED0] font-bold uppercase tracking-widest">
              Mapa de Fluxo
            </div>
          </div>
        </div>
