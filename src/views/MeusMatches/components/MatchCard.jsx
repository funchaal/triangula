...
function MatchCard({ match, isActive, labels, tipo, confirmed, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`
        cursor-pointer rounded-2xl sm:rounded-3xl border transition-all duration-300 p-3 sm:p-4 relative overflow-hidden group
        shrink-0 w-[85vw] sm:w-[320px] lg:w-full snap-center bg-[#13204c]
        ${isActive
          ? 'border-blue-500 bg-blue-500/10 ring-1 ring-blue-500/50'
          : 'border-white/5 bg-[#0B1437] md:hover:brightness-110'
        }
      `}
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
        {labels.map((label, i) => (
          <div key={i} className="flex items-center gap-1.5 sm:gap-2">
            <div className={`px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-semibold leading-tight text-center transition-colors ${
              isActive
                ? 'bg-blue-500 text-white shadow-md shadow-blue-500/20'
                : 'bg-white/5 text-[#A3AED0] md:group-hover:text-white'
            }`}>
              {label}
            </div>
            ...
            {i < labels.length - 1 && (
              <ArrowLeftRight size={14} className={`shrink-0 sm:w-3.5 sm:h-3.5 ${isActive ? "text-blue-400" : "text-[#A3AED0]/50"}`} />
            )}
          </div>
        ))}
      </div>
