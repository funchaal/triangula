...
// Constrói payload com APENAS os campos que mudaram em relação ao user salvo
function buildDiffPayload(profile, user) {
  ...
  // Localização (base é string, region/state são int)
...
function MeusDados() {
  ...
  // Validação: Impede salvar se houver campo obrigatório em branco ou igual a ANY
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      console.error('[MeusDados] Falha ao salvar:', e);
    }
  };
  ...
  // Componente visual para campos em modo leitura
  const ReadOnlyText = ({ text, fallback = "—" }) => (
    <div className="w-full bg-slate-900/40 border border-slate-800/60 rounded-lg px-4 py-2.5 flex items-center min-h-[42px] mt-1.5 transition-colors">
      {text ? (
        ...
        <span className="text-sm text-slate-500 italic">{fallback}</span>
      )}
    </div>
  );
      ...
      <div>
        ...
        {isEditing ? (
          <div className="mt-1.5">
            <Select value={profile.state_id} onChange={handleProfileState} disabled={profile.base_id !== ANY || profile.region_id !== ANY}>
              <option value={ANY}>-</option>
              {stateOpts.sort((a, b) => a[1].name.localeCompare(b[1].name))
                .map(([id, s]) => <option key={id} value={id}>{s.name}</option>)}
            </Select>
          </div>
        ) : (
          <ReadOnlyText text={profile.state_id !== ANY ? states[profile.state_id]?.name : null} fallback="Não informado" />
        )}
      </div>
      <div>
        <Label>Região</Label>
        {isEditing ? (
          <div className="mt-1.5">
            <Select value={profile.region_id} onChange={handleProfileRegion} disabled={profile.base_id !== ANY}>
              <option value={ANY}>-</option>
              {regionsForProfile.sort((a, b) => a[1].name.localeCompare(b[1].name))
                .map(([id, r]) => <option key={id} value={id}>{r.name}</option>)}
            </Select>
          </div>
        ) : (
          <ReadOnlyText text={profile.region_id !== ANY ? regions[profile.region_id]?.name : null} fallback="Não informado" />
        )}
      </div>
      <div>
        <Label>Base</Label>
        {isEditing ? (
          <div className="mt-1.5">
            <Select value={profile.base_id} onChange={handleProfileBase}>
              <option value={ANY}>-</option>
              {basesForProfile.sort((a, b) => a[1].name.localeCompare(b[1].name))
                .map(([id, l]) => <option key={id} value={id}>{l.name}</option>)}
            </Select>
          </div>
        ) : (
          <ReadOnlyText text={profile.base_id !== ANY ? locations[profile.base_id]?.name : null} fallback="Não informado" />
        )}
      </div>
    </div>
  );
  ...
  const ProfSelects = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-4">
      {[
        { f: 'role_id',       l: 'Cargo / Ênfase',   opts: roleOpts   },
        { f: 'role_type_id',  l: 'Nível',            opts: rtOpts     },
        { f: 'department_id', l: 'Gerência / Depto.', opts: deptOpts   },
        { f: 'regime_id',     l: 'Regime',            opts: regimeOpts },
      ].map(({ f, l, opts }) => {
        const val  = profile[f] ?? ANY;
        const name = opts.find(([id]) => String(id) === String(val))?.[1];
        const displayName = val !== ANY && name ? (typeof name === 'string' ? name : name?.name) : null;
        return (
          <div key={f}>
            <Label>{l}</Label>
            {isEditing ? (
              <div className="mt-1.5">
                <Select value={val} onChange={e => setProfile(p => ({ ...p, [f]: e.target.value }))}>
                  <option value={ANY}>-</option>
                  {opts.map(([id, n]) => <option key={id} value={id}>{typeof n === 'string' ? n : n?.name}</option>)}
                </Select>
              </div>
            ) : (
              <ReadOnlyText text={displayName} fallback="Não informado" />
            )}
          </div>
        );
      })}
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 overflow-hidden">
      {/* Estilos para customizar e separar a bandeira do input */}
      <style>{`
        .PhoneInput {
          display: flex;
          align-items: center;
        ...
        .PhoneInputCountry {
          ...
          align-items: center;
          gap: 0.5rem;
          background-color: rgba(15, 23, 42, 1); /* bg-slate-900 */
          border: 1px solid rgba(51, 65, 85, 1); /* border-slate-700 approx */
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex justify-between items-center h-full">
            ...
            {isEditing && (
              <div className="hidden sm:flex items-center gap-3">
                <ButtonGhost onClick={() => { setIsEditing(false); resetProfile(); }}>
                  Cancelar
                </ButtonGhost>
                <ButtonPrimary
                  onClick={handleSaveProfile}
                  disabled={isSavingProfile || !profileIsDirty || !isProfileValid}
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 pb-24 sm:pb-8
          [&::-webkit-scrollbar]:w-1
          [&::-webkit-scrollbar-track]:bg-transparent
          [&::-webkit-scrollbar-thumb]:bg-slate-800
          [&::-webkit-scrollbar-thumb]:rounded-full">

          {/* Dica do Sistema Destacada no Topo */}
          <div className="bg-gradient-to-r from-amber-900/20 to-transparent border border-amber-500/20 rounded-xl p-4 flex flex-col sm:flex-row gap-3 sm:gap-4 items-start shadow-sm">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs md:text-sm text-slate-300/90 mt-1.5 leading-relaxed">
                Mantenha as informações sempre atualizadas. <strong>Não deixe campos em branco</strong>, especialmente os de lotação e perfil profissional, pois o sistema utiliza esses dados exatos para encontrar e validar seus matches.
              </p>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between gap-4 mb-8">
              <div className="flex items-center gap-4 md:gap-5 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isEditing ? (
                    <div className="mt-1.5">
                      <Input value={profile.name} onChange={e => setProfile(f => ({ ...f, name: e.target.value }))} placeholder="Nome completo" className="w-full" />
                    </div>
                  ) : (
                    <p className="text-sm text-slate-400 mt-1 truncate">{profile.name || <span className="text-slate-600 italic">Sem nome cadastrado</span>}</p>
                  )}
                </div>
              </div>
              {!isEditing && (
            ...
            <div className="mt-6 md:mt-8 space-y-4 md:space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-4">
                ...
                <div>
                  <Label>E-mail</Label>
                  {isEditing
                    ? <div className="mt-1.5"><Input type="email" value={profile.email} onChange={e => setProfile(f => ({ ...f, email: e.target.value }))} placeholder="seu@email.com" /></div>
            ...
            <div className="mt-6 md:mt-8 space-y-3">
              ...
              {isEditing ? (
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-blue-500 transition-all resize-none placeholder:text-slate-700 mt-1.5"
                />
              ) : (
                <div className="w-full bg-slate-900/40 border border-slate-800/60 rounded-lg px-4 py-3 text-sm text-slate-300 min-h-[80px] whitespace-pre-wrap mt-1.5 italic">
                  {profile.observations || <span className="text-slate-600">Nenhuma observação cadastrada.</span>}
                </div>
              )}
            </div>

            {/* Botões — fixo no mobile com blur, oculto no desktop */}
            {isEditing && (
              <div className="fixed bottom-0 left-0 w-full p-4 bg-slate-950/80 backdrop-blur-md border-t border-slate-800 z-50 flex gap-3 sm:hidden shadow-[0_-10px_20px_rgba(0,0,0,0.3)]">
                <ButtonGhost
                  ...
                  className="flex-1 justify-center bg-slate-900"
                >
                  Cancelar
                </ButtonGhost>
                <ButtonPrimary
...
function SectionTitle({ icon, label }) {
  return (
    <div className="flex items-center gap-2.5 text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-500">
